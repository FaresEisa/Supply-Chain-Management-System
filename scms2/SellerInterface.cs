...
namespace scms2.cs
{
    internal class SellerInterface
    {
        ...
        private static int ChooseItem(int subcategoryId)
        {
            ...
            {
                conn.Open();
                string query = "SELECT Id, ItemName, Stock FROM Items WHERE SubcategoryId = @SubcategoryId ORDER BY Id";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
        ...
        // Method to update the item stock (sum of all sellers' stock)
        private static void UpdateItemStock(int itemId, int stockToAdd)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Calculate the total stock from all sellers for this item
                string totalStockQuery = @"
            SELECT SUM(Quantity) 
            FROM SellerStock 
            WHERE ItemName = (SELECT ItemName FROM Items WHERE Id = @ItemId)";

                using (SqlCommand totalStockCmd = new SqlCommand(totalStockQuery, conn))
                {
                    totalStockCmd.Parameters.AddWithValue("@ItemId", itemId);
                    object totalStockResult = totalStockCmd.ExecuteScalar();
                    int totalStock = totalStockResult != DBNull.Value ? Convert.ToInt32(totalStockResult) : 0;

                    // Add the stock for the current seller to the total stock
                    totalStock += stockToAdd;

                    // Update the stock in the Items table with the new total stock
                    string updateQuery = "UPDATE Items SET Stock = @TotalStock WHERE Id = @ItemId";
